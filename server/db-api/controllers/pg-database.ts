...
export const getRecipe = async (req: Request, res: Response) => {
    ...
    try {
        const recipeResult = await pool.query(`SELECT * FROM Recipe WHERE RecipeId = '${id}';`);
        const recipes: Recipe[] = recipeResult.rows;
        const ingredientResult = await pool.query(`SELECT * FROM Ingredient WHERE RecipeId = '${id}' ORDER BY index ASC;`);
